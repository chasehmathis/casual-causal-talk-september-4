...
and we will have a Gaussian Copula between $X$ and $Y$ with parameter
$\rho = 2\operatorname{expit}(0.4) - 1$
...
### Plasmode Example
Plasmode means to simulate on top of existing data. Suppose you want 
to run an experiment on students in the Berkeley statistics department assigning
them to either come into Evans hall or stay home to work. An interesting outcome could
be time till graduation (we'll return to this), but an easier one is their dat
grade average at the end of the year (continuous, normal assumption). For returning
students we have their grades from last year, otherwise we have 1st year students grades
from their last school. We have this information it is fixed and not random.
Say, we want to assign treatment conditional on that. 
